...
class Board:
    ...
    def move_piece(self, sqSelected,sqDest):
        ...
        if self.isMax :
         ...
         move_dict = move_generator.legalMoves(self.chessBoard, positions, self.isMax,self.can_castle_white )
        if self.isMax== False:
            positions = np.where(self.chessBoard < 0)
            move_dict = move_generator.legalMoves(self.chessBoard, positions, self.isMax, self.can_castle_black)
        ...
        if  move_dict[sqSelected].__contains__(sqDest):
           ...
           if self.can_castle_white :
             ...
             if sqSelected == (7,4) or  sqSelected == (7,0) or sqSelected == (7,7) :
                 self.can_castle_white= False

           if self.can_castle_black:
               ...
               if   sqSelected == (0, 4) or sqSelected == (0,0) or sqSelected == (0,7):
